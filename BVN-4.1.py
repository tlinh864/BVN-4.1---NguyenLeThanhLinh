...
def caesar_encrypt(plaintext, k):
    ...
    for char in plaintext:
        if char.isalpha():  # Kiểm tra ký tự là chữ
            ...
            # Mã hóa ký tự
            encrypted_char = chr((ord(char) - base + k) % 26 + base) # Công thức mã hóa Caesar
...
def caesar_decrypt(ciphertext, k):
    ...
    for char in ciphertext:
        if char.isalpha():
            base = ord('A') if char.isupper() else ord('a')
            decrypted_char = chr((ord(char) - base - k) % 26 + base) # Công thức giải mã Caesar
...
ciphertext = caesar_encrypt(plaintext, k)
print("Plaintext :", plaintext)  
print("Ciphertext:", ciphertext)    # Ciphertext: XqeioxVoDrkxrVsxr
